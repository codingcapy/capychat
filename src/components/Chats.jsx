...
export default function Chats(props) {
    ...
    return (
        <div className="px-4 flex-2 border-2 border-slate-600 min-w-72 h-[89vh] md:h-screen overflow-y-auto ">
            ...
            <div>
                {props.chats && props.chats.map((chat) => <div key={chat.chat_id} onClick={() => props.clickedChat(chat)} className="py-2 px-1 cursor-pointer hover:bg-slate-600 transition-all ease duration-300">
                    {chat.title}
                </div>)}
            </div>
        </div>
